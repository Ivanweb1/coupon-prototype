...
/* Заливка круга — #F9F9F9, цвет Ивана (22.09.2026). Обводки у круга нет,
   форму держит сама заливка, настолько светлая, чтобы не спорить со
   знаком внутри. */
const CAT_TONES_MONO = ["#F9F9F9", "#DD443C"];
...
function renderCouponDesign() {
  ...
  qs("#about").innerHTML = `
    <div class="cpd-about__main">
      ...
      <!-- Длинное описание — в три строки и «Показать ещё» (Иван 25.09).
           Компания пишет его сама в кабинете, и длина там не ограничена:
           без сворачивания абзац в десять строк сталкивал точки продаж
           вниз. Кнопка появляется, только если текст правда не влез. -->
      <div class="cpd-lead" data-lead>
        <p class="cpd-about__lead">${c.company} ${c.market
          ? "продаёт на " + c.market + " с 2021 года. Отгрузка со склада площадки, возврат по правилам маркетплейса. " +
            "Все товары сертифицированы, на технику — гарантия производителя от года. Отвечаем на вопросы в чате площадки " +
            "в течение часа, в выходные тоже. Упаковываем с запасом прочности: за последний год ни одного возврата из-за повреждений."
          : "работает в городе с 2014 года. Своя команда, собственное оборудование, запись день в день. " +
            "Мастера регулярно проходят обучение, работаем только на профессиональных материалах. " +
            "Для гостей — парковка у входа, детский уголок и кофе, пока ждёте. " +
            "Если что-то не понравилось, переделаем бесплатно в течение недели."}</p>
        <button type="button" class="cpd-lead__more" data-lead-more hidden>Показать ещё</button>
      </div>
  ...
  /* «Показать ещё» у описания — только если оно не влезло в три строки */
  const lead = qs("[data-lead]");
  if (lead) {
    const p = qs(".cpd-about__lead", lead);
    const more = qs("[data-lead-more]", lead);
    more.hidden = p.scrollHeight <= p.clientHeight + 1;
    more.onclick = () => {
      const open = lead.classList.toggle("is-open");
