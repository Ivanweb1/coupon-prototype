...
window.LK_NICHES = {
  regional: [
    ...
    /* 18+ (созвон 14.09): на витрине под блюром с подтверждением возраста,
       в соцсети и рекомендации не уходит */
    { name: "18+",              base: 1400, adult: true }
  ],
  marketplace: [
...
/* Точки продаж компании — те же, что в профиле. Из них клиент выбирает,
   где действует купон; новый адрес дописывается сюда (созвон 16.09). */
window.LK_ADDRESSES = [
  { city: "Липецк", addr: "ул. Первомайская, 12" },
  { city: "Липецк", addr: "пр-т Победы, 45" }
];

/* Подсказки поиска адреса. В продукте — живой поиск по базе адресов,
   здесь — рыба на три города запуска. */
window.LK_ADDRESS_SUGGEST = [
  { city: "Липецк", addr: "ул. Советская, 64" },
  { city: "Липецк", addr: "ул. Гагарина, 27" },
  { city: "Липецк", addr: "пл. Героев, 1" },
  { city: "Елец",   addr: "ул. Коммунаров, 30" },
  { city: "Елец",   addr: "ул. Мира, 112" },
  { city: "Грязи",  addr: "ул. Красная площадь, 9" }
];

/* Попыток генерации изображения на один купон. На созвоне 16.09 назвали
   5–10, точное число не закрепили. */
window.LK_GEN_LIMIT = 10;
...
/* Бонусы, которые скоро сгорят — плашка на дашборде (эскиз 14.09) */
window.LK_BONUS_BURN_SOON = { amount: 500, date: "30 сентября" };

/* Компания клиента: шапка кабинета и поля, которые подставляются в купон
   из регистрации (созвон 10.09 — соцсети предзаполнены, но правятся). */
window.LK_COMPANY = {
  name: "Кофейня «Пример»", ava: "КП", inn: "4826000000", city: "Липецк",
  niche: "Кафе и рестораны",
  site: "https://primer-coffee.ru", vk: "https://vk.com/primer_coffee", tg: "https://t.me/primer_coffee"
};

/* Способы получить бонусы — попап «Получить бонусы» (созвон 14.09).
   Суммы — рыба. once — механика срабатывает один раз; monthly — раз в
   месяц, после использования кнопка серая до следующего месяца. */
window.LK_BONUS_WAYS = [
  { id: "register", title: "Бонус за регистрацию",            reward: "500",           note: "Разово",               act: "Получить",      once: true },
  { id: "survey",   title: "Анкета о компании",                reward: "700",           note: "Разово, 5 минут",      act: "Заполнить",     once: true },
  { id: "friend",   title: "Пригласить друга",                 reward: "300 за друга",  note: "Когда друг разместит первый купон", act: "Скопировать ссылку" },
  { id: "repost",   title: "Репост ссылки на свой купон",      reward: "100",           note: "За каждый купон",      act: "Поделиться" },
  { id: "review",   title: "Отзыв о сервисе",                  reward: "200",           note: "Разово",               act: "Написать",      once: true },
  { id: "partner",  title: "Запросить у регионального партнёра", reward: "до 1 000",    note: "Раз в месяц",          act: "Запросить",     monthly: true }
];

/* Топовые механики в нише клиента (эскиз 14.09). Пока статистики нет —
   заготовки команды; «Создать купон» открывает конструктор с этой
   механикой. */
window.LK_TOP_MECHANICS = [
  { niche: "Кафе и рестораны", mech: "gift",      effect: "Больше всего новых гостей" },
  { niche: "Кафе и рестораны", mech: "twoforone", effect: "Приходят компанией — выше чек" },
  { niche: "Кафе и рестораны", mech: "friend",    effect: "Гости сами приводят новых" }
];

/* Во сколько раз соцсеть увеличивает просмотры купона (созвон 10.09):
   числа от 3 до 5, между каналами не больше 0,5, по нишам свои. keep —
   какая доля компаний ниши оставляет канал включённым. Всё — рыба. */
window.LK_REACH = {
  base:              { vk: 3.9, ok: 3.5, tg: 3.7, max: 3.4 },
  "Кафе и рестораны":{ vk: 4.2, ok: 3.8, tg: 4.0, max: 3.7 },
  "Красота":         { vk: 4.4, ok: 3.9, tg: 4.1, max: 3.9 },
  "Товары для дома": { vk: 3.6, ok: 3.9, tg: 3.4, max: 3.3 }
};
window.LK_REACH_KEEP = { vk: 98, ok: 81, tg: 92, max: 74 };

/* Рекомендации конструктора (созвон 10.09): какая механика лучше в нише
   и какой срок советуем. Рыба до аналитики. */
window.LK_MECH_ADVICE = {
  "Кафе и рестораны": { mech: "gift",      why: "больше всего новых гостей" },
  "Красота":          { mech: "percent",   why: "чаще записываются на первый визит" },
  base:               { mech: "twoforone", why: "купон чаще забирают" }
};
window.LK_RECOMMENDED_DAYS = 30;

/* Проверка тайным покупателем (созвон 10.09): цена за каждый город
   размещения, доверие к купону — «в 6 раз», срок визита 3–5 дней. */
window.LK_SECRET = { price: 5000, trust: 6, days: "3–5 дней" };
